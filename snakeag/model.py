...
class QTrainer1:
    def __init__(self, model1,model2, lr, gamma):
        ...
        self.model1 = model1
        self.model2=model2
        self.optimizer = optim.Adam(model1.parameters())
...
class QTrainer2:
    ...
    def train_step(self, state, action, reward, next_state, done):
        ...
        coefficient=0.5
        with torch.no_grad():
          for target_param, q_param in zip(self.model2.parameters(),self.model1.parameters()):
             target_param.data = coefficient * q_param.data + (1 - coefficient) * target_param.data
